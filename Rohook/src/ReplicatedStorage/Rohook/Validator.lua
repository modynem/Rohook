...
function Validator:Url(url: string): (boolean, string?)
	if typeof(url) ~= "string" then
		return false, "Input is not a string: " .. typeof(url)
	end
	local isValid = string.match(url, "^https://discord%.com/api/webhooks/%d+/[%w%-]+$") ~= nil
	if isValid then
		return true, nil
	else
		return false, "URL does not match Discord webhook pattern: " .. url
	end
end
